...
int main(void)
{
    ...
    // In float precision: nvcc -O3 -arch=sm_75 neighbor2gpu.cu -o neighbor2gpu.out
    // In double precision: nvcc -O3 -arch=sm_75 -DUSE_DP neighbor2gpu.cu -o neighbor2gpu.out
    // Run exe file: cuda-memcheck ./neighbor2gpu.out
                   ...
                   const int N, const real cutoff_square)
{
    ...
    for (int n2 = n1 + 1; n2 < N; ++n2)
    {
        real x12 = d_x[n2] - x1;
        real y12 = d_y[n2] - y1;
        real distance_square = x12 * x12 + y12 * y12;  // faster than using sqrt
        if (distance_square < cutoff_square)
        {
            d_NL[n1 * MN + atomicAdd(&d_NN[n1], 1)] = n2;
            d_NL[n2 * MN + atomicAdd(&d_NN[n2], 1)] = n1;

            // another version; atomicAdd must be first to record old value in case of being changed by other threads
            // int nn1 = atomicAdd(&d_NN[n1], 1);
            // int nn2 = atomicAdd(&d_NN[n2], 1);
            // d_NL[n1 * MN + nn1] = n2;
            // d_NL[n2 * MN + nn2] = n1;
        }
    }
                   ...
                   const int N, const real cutoff_square)
{
    ...
    if (n1 >= N)
        return;
    int count = 0;  // use count to avoid visiting global memory too many times
    real x1 = d_x[n1];
    real y1 = d_y[n1];
    for (int n2 = 0; n2 < N; ++n2)
    {
        ...
        real y12 = d_y[n2] - y1;
        real distance_square = x12 * x12 + y12 * y12;  // faster than using sqrt
        // distance first to minish the number of judgements

...
int main(void)
{
    ...
    // In float precision: nvcc -O3 -arch=sm_75 neighbor1cpu.cu -o neighbor1cpu.out
    // In double precision: nvcc -O3 -arch=sm_75 -DUSE_DP neighbor1cpu.cu -o neighbor1cpu.out
    // Run exe file: cuda-memcheck ./neighbor1cpu.out
...
void read_xy(std::vector<real>& v_x, std::vector<real>& v_y)
{
    ...
    {
        std::istringstream words(line);  // create a string stream of the current line
        if(line.length() == 0)
        {
            continue;
        }
        for (int i = 0; i < 2; i++)
        {
            if(words >> word)  // >> operator extracts and parses characters sequentially from the stream
            {
                if(i == 0)
                {

...
int main(void)
{
    ...
    // In float precision: nvcc -O3 -arch=sm_75 reduce2gpu.cu -o reduce2gpu.out
    // In double precision: nvcc -O3 -arch=sm_75 -DUSE_DP reduce2gpu.cu -o reduce2gpu.out
    // Run exe file: cuda-memcheck ./reduce2gpu.out
...
void __global__ reduce_global(real *d_x, real *d_y)
{
    ...
    for (int offset = blockDim.x >> 1; offset > 0; offset >>= 1)
    {
        if (tid < offset)
        {
            // no examination of tid < N, 
            // so assuming N is a multiple of blockDim.x
            x[tid] += x[tid + offset];
        }
        __syncthreads();  // ensure all threads in one block have arrived here
    }

...
int main(void)
{
    ...
    // In float precision: nvcc -O3 -arch=sm_75 reduce1parallelism.cu -o reduce1parallelism.out
    // In double precision: nvcc -O3 -arch=sm_75 -DUSE_DP reduce1parallelism.cu -o reduce1parallelism.out
    // Run exe file: cuda-memcheck ./reduce1parallelism.out
...
void __global__ reduce_cp(const real *d_x, real *d_y, const int N)
{
    ...
    // shrink d_x from N to GRID_SIZE * BLOCK_SIZE
    real y = 0.0;

...
int main(void)
{
    ...
    // In float precision: nvcc -O3 -arch=sm_75 reduce.cu -o reduce.out
    // In double precision: nvcc -O3 -arch=sm_75 -DUSE_DP reduce.cu -o reduce.out
    // Run exe file: cuda-memcheck ./reduce.out
...
void __global__ reduce_shfl(const real *d_x, real *d_y, const int N)
{
    ...
    for (int offset = 16; offset > 0; offset >>= 1)
    {
        // shouldn't use if statement here, because __shfl_down_sync assumes
        // all threads in the warp are active. using if statement will deactivate
        // threads with tid >= offset and lead to undefined behavior.
        y += __shfl_down_sync(FULL_MASK, y, offset);
        // y += __shfl_xor_sync(FULL_MASK, y, offset);
    }
    ...
    // for (int offset = 16; offset > 0; offset >>= 1)
    // {
    //     s_y[tid] += __shfl_down_sync(0xffffffff, s_y[tid], offset);
    // }
...
void __global__ reduce_cp(const real *d_x, real *d_y, const int N)
{
    ...
    {
        // g is a thread_block_tile<32> object, which is a group of 32 threads, 
        // and g here denotes the group that tid exists in.
        y += g.shfl_down(y, i);
    }
